...
const state={score:0,correctAnswers:0,answered:new Set(),lockedQuestions:new Set(),level:'easy',timeLeft:0,timer:null,currentQuestions:[]};
...
function answerQuestion(questionId,choiceIndex,buttonEl){if(state.lockedQuestions.has(questionId))return;const q=state.currentQuestions.find(x=>x.id===questionId);if(!q)return;const selected=q.choices[choiceIndex];const box=document.getElementById('question-'+questionId);const buttons=box.querySelectorAll('button');const feedback=document.getElementById('feedback-'+questionId);const correctIndex=q.choices.findIndex(c=>c.correct);buttons.forEach(btn=>btn.disabled=true);state.lockedQuestions.add(questionId);state.answered.add(questionId);if(selected.correct){state.correctAnswers+=1;state.score=Math.round((state.correctAnswers/state.currentQuestions.length)*MAX_SCORE);buttonEl.classList.add('answer-correct');feedback.innerHTML='✅ <strong>Correct.</strong> '+q.explanation;}else{buttonEl.classList.add('answer-incorrect');if(buttons[correctIndex]){buttons[correctIndex].classList.add('answer-correct');}feedback.innerHTML='❌ <strong>Incorrect.</strong> '+q.explanation;}feedback.classList.add('show');updateDashboard();if(state.answered.size>=state.currentQuestions.length)finishTraining('You answered every question.');}
function updateDashboard(){document.getElementById('scoreValue').textContent=`${state.score}/${MAX_SCORE}`;document.getElementById('mobileScoreValue').textContent=`${state.score}/${MAX_SCORE}`;document.getElementById('answeredValue').textContent=String(state.answered.size);let status='Beginner';if(state.score>=300)status='Advanced';if(state.score>=600)status='Sharp-eyed';if(state.score>=900)status='Security Aware';document.getElementById('statusValue').textContent=status;}
function finishTraining(reason){clearInterval(state.timer);state.timer=null;const total=state.currentQuestions.length||getQuestionsForLevel(state.level).length;const resultBox=document.getElementById('resultBox');resultBox.classList.remove('hidden');resultBox.innerHTML=`<h3>🏁 Training Complete</h3><p><strong>Status:</strong> ${reason}</p><p><strong>Score:</strong> ${state.score} out of ${MAX_SCORE}</p><p><strong>Correct answers:</strong> ${state.correctAnswers} out of ${total}</p><p><strong>Difficulty:</strong> ${LEVEL_CONFIG[state.level].label}</p>`;}
